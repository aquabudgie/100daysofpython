...
def play_round():
    ...
    computer_cards = random.choices(card_list, k = 2)
    computer_score = sum(computer_cards)
    print(f"    Your cards: {player_cards} current score: {player_score}")
    print(f"    Computer's first card: {computer_cards[0]}")
    while player_score < 21:
        add_card = input("\nWould you like to get another card? Type 'y' or 'n': ").lower()
        if add_card == 'y':
    ...
    # print( if player_score >= 21:
    # end_game
    return


def main():
    play_game = input("\nDo you want to play a game of Backjack? Type 'y' or 'n': ").lower()
    if play_game == "y":
        play_round()
    else:
        exit()
